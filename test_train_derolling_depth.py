...
from marigold.derolling_depth_pipeline import MarigoldRGBDepthPipeline
from marigold.marigold_pipeline import MarigoldPipeline
from src.dataset import BaseRGBDepthDataset, DatasetMode, get_dataset
...
if "__main__" == __name__:
    ...
    train_dataset: BaseRGBDepthDataset = get_dataset(
        cfg_data.train,
    ...
    # model = MarigoldRGBDepthPipeline.from_pretrained(
    #     os.path.join(base_ckpt_dir, cfg.model.pretrained_path), unet_marigold=None, unet_derolling=None, **_pipeline_kwargs
    # )
    # model_copy = MarigoldPipeline.from_pretrained(
    #     "prs-eth/marigold-depth-v1-1", **_pipeline_kwargs)
    
    # if 8 != model_copy.unet.config["in_channels"]:
    #     _replace_unet_conv_in(model)

    # model.unet_marigold = model_copy.unet
    # model.unet_derolling = UNet2DConditionModel.from_config(model_copy.unet.config)


# Load pretrained UNet for marigold
    model_copy = MarigoldPipeline.from_pretrained("prs-eth/marigold-depth-v1-1", **_pipeline_kwargs)
    ...
    pipe_pretrained = MarigoldPipeline.from_pretrained("prs-eth/marigold-depth-v1-1", **_pipeline_kwargs)
    ...
    model.unet_derolling.to(device)
    model.unet_marigold.to(device)
    #From pretrained!!!
